export function parseQuestionsCsv(csvString) {
    const errors = [];
    const data = [];
    let columnHeaders = [];
    ...
    questions.forEach(q => {
        ...
        if (q.type === 'multiple-choice' && typeof q.correctAnswer === 'number') {
            ...
            if (q.correctAnswer === 0)
                correctAnswerLetter = 'A'; // Assuming option A is True
            else if (q.correctAnswer === 1)
                correctAnswerLetter = 'B'; // Assuming option B is False
        }
