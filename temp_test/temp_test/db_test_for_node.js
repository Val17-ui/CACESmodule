...
const db_1 = require("../src/db"); // Adjusted path and extension
// Define a QuestionType enum locally for test data, mirroring what might be in types.ts or the actual string literals
var QuestionTypeForTest;
(function (QuestionTypeForTest) {
    QuestionTypeForTest["QCM"] = "multiple-choice";
    // QCU = 'multiple-choice', // Assuming QCU is a variant of multiple-choice for data structure
    QuestionTypeForTest["TRUE_FALSE"] = "true-false";
})(QuestionTypeForTest || (QuestionTypeForTest = {}));
...
const runDBTests = async () => {
    ...
    try {
        if (typeof fetch !== 'function' || typeof Blob !== 'function') {
            ...
            if (!response.ok)
                throw new Error(`Failed to fetch base64 image, status: ${response.status}`);
    ...
    const sampleQuestion = {
        ...
        timeLimit: 30,
        isEliminatory: false,
        referential: 'R489', // Use string literal, cast for type safety
        theme: 'reglementation', // Use string literal, cast for type safety
        image: mockImageBlob,
    ...
    try {
        newQuestionId = await (0, db_1.addQuestion)(sampleQuestion);
    ...
    try {
        const allQuestions = await (0, db_1.getAllQuestions)();
    ...
    if (newQuestionId !== undefined) {
        try {
            const updatedId = await (0, db_1.updateQuestion)(newQuestionId, { text: updatedText });
            if (updatedId !== undefined) {
                log(`Update initiated for question with ID ${updatedId}.`);
                const updatedQuestion = await (0, db_1.getQuestionById)(newQuestionId);
    ...
    if (newQuestionId !== undefined) {
        try {
            await (0, db_1.deleteQuestion)(newQuestionId);
            log(`Deletion initiated for question with ID ${newQuestionId}.`);
            const deletedQuestion = await (0, db_1.getQuestionById)(newQuestionId);
