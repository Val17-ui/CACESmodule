...
import {
  ...
  transformParsedResponsesToSessionResults,
...
const SessionForm: React.FC<SessionFormProps> = ({ sessionIdToLoad }) => {
  const [currentSessionDbId, setCurrentSessionDbId] = useState<number | null>(
    ...
    if (sessionIdToLoad) {
      const loadSession = async () => {
        ...
        if (sessionData) {
          ...
          setSessionDate(
            sessionData.dateSession ? sessionData.dateSession.split("T")[0] : ""
          );
          setSelectedReferential(
  ...
  const referentialOptions = Object.entries(referentials).map(
    ([value, label]) => ({
      value,
      label: `${value} - ${label}`,
    })
  ...
  const prepareSessionDataForDb = async (
    includeOrsBlob?: Blob | null
  ): Promise<DBSession | null> => {
    if (!selectedReferential && !currentSessionDbId) {
      alert(
        "Veuillez sélectionner un référentiel CACES pour une nouvelle session."
      );
      return null;
    }
    ...
    const sessionToSave: DBSession = {
      ...
      nomSession:
        sessionName || `Session du ${new Date().toLocaleDateString()}`,
      dateSession: sessionDate || new Date().toISOString().split("T")[0],
      referentiel: selectedReferential || sessionToUpdate?.referentiel || "",
      participants: dbParticipants,
      selectionBlocs: Object.entries(selectedBlocksSummary).map(
        ([theme, blockId]) => ({ theme, blockId })
      ),
      donneesOrs:
        includeOrsBlob !== undefined
          ? includeOrsBlob
          : sessionToUpdate?.donneesOrs,
      location: location,
    ...
    try {
      ...
      if (sessionData.id) {
        ...
        if (newId) {
          ...
          setImportSummary(
            "Erreur critique : La nouvelle session n'a pas pu être créée."
          );
          return null;
        }
      }
      if (savedId) {
        ...
        if (reloadedSession) {
          ...
          if (reloadedSession.selectionBlocs) {
            reloadedSession.selectionBlocs.forEach((sb) => {
              summary[sb.theme] = sb.blockId;
            });
          }
          setSelectedBlocksSummary(summary);
  ...
  const handleGenerateQuestionnaireAndOrs = async () => {
    if (!selectedReferential) {
      setImportSummary("Veuillez sélectionner un référentiel.");
      return;
    }
    if (!templateFile) {
      setImportSummary("Veuillez sélectionner un fichier modèle PPTX.");
      return;
    }
    ...
    try {
      const baseThemes = await StorageManager.getAllBaseThemesForReferential(
        selectedReferential
      );
      if (baseThemes.length === 0) {
        setImportSummary(`Aucun thème trouvé pour ${selectedReferential}.`);
        return;
      }
      for (const baseTheme of baseThemes) {
        const blockIdentifiers =
          await StorageManager.getAllBlockIdentifiersForTheme(
            selectedReferential,
            baseTheme
          );
        if (blockIdentifiers.length === 0) {
          console.warn(`Aucun bloc pour ${baseTheme}`);
          continue;
        }
        const chosenBlockIdentifier =
          blockIdentifiers[Math.floor(Math.random() * blockIdentifiers.length)];
        tempSelectedBlocksSummary[baseTheme] = chosenBlockIdentifier;
        const questionsFromBlock = await StorageManager.getQuestionsForBlock(
          selectedReferential,
          baseTheme,
          chosenBlockIdentifier
        );
        allSelectedQuestionsForPptx =
          allSelectedQuestionsForPptx.concat(questionsFromBlock);
      }
      setSelectedBlocksSummary(tempSelectedBlocksSummary);
      if (allSelectedQuestionsForPptx.length === 0) {
        setImportSummary("Aucune question sélectionnée.");
        return;
      }
      ...
      const adminSettings: AdminPPTXSettings = {
        defaultDuration: 30,
        pollTimeLimit: 30,
        answersBulletStyle: "ppBulletAlphaUCPeriod",
      ...
      if (
        generationOutput &&
        generationOutput.orsBlob &&
        generationOutput.questionMappings
      ) {
        ...
        setImportSummary(
          "Erreur génération .ors/mappings. Données manquantes."
        );
      }
    } catch (error: any) {
  ...
  const handleImportResults = async () => {
    if (!resultsFile) {
      setImportSummary("Veuillez sélectionner un fichier de résultats.");
      return;
    }
    if (!currentSessionDbId || !editingSessionData) {
      setImportSummary(
        "Aucune session active ou données de session non chargées."
      );
      return;
    }
    ...
    try {
      ...
      if (!orSessionXmlFile) {
        setImportSummary("Erreur: ORSession.xml introuvable.");
        return;
      }
      ...
      setImportSummary(
        `${extractedResults.length} réponses extraites. Transformation...`
      );
      ...
      if (!currentQuestionMappings || currentQuestionMappings.length === 0) {
        setImportSummary("Erreur: Mappages questions manquants.");
        return;
      }
      ...
      if (sessionResultsToSave.length > 0) {
        try {
          const savedResultIds = await addBulkSessionResults(
            sessionResultsToSave
          );
                ...
                if (sessionDataForScores && sessionResultsForScore.length > 0) {
                  ...
                  sessionResultsForScore.forEach((sr: SessionResult) => {
                    ...
                    scoresByParticipant.set(
                      sr.participantIdBoitier,
                      currentScore + (sr.pointsObtained || 0)
                    );
                  });
                  ...
                  const updatedParticipants =
                    sessionDataForScores.participants.map((p) => {
                      const participantScore = scoresByParticipant.get(
                        p.idBoitier
                      );
                      if (participantScore !== undefined) {
                        participantsActuallyUpdated = true;
                        const reussite =
                          participantScore >= SEUIL_REUSSITE_SCORE;
                        return {
                          ...p,
                  ...
                  if (participantsActuallyUpdated) {
                    ...
                    const finalUpdatedSession = await getSessionById(
                      currentSessionDbId
                    );
                    if (finalUpdatedSession) {
                      setEditingSessionData(finalUpdatedSession);
                      const formParticipantsToUpdate: FormParticipant[] =
                        finalUpdatedSession.participants.map((p_db, index) => ({
                          ...p_db,
                          id:
                            participants[index]?.id ||
                            `updated-${index}-${p_db.idBoitier}`,
                          firstName: p_db.prenom,
                          lastName: p_db.nom,
                          deviceId:
                            participants[index]?.deviceId ||
                            parseInt(p_db.idBoitier, 10) ||
                            index + 1,
                          organization:
                            participants[index]?.organization ||
                            (p_db as any).organization,
                          hasSigned:
                            participants[index]?.hasSigned ||
                            (p_db as any).hasSigned,
                        }));
          ...
          setImportSummary(
            `Erreur DB sauvegarde résultats: ${dbError.message}`
          );
        }
      } else {
  ...
  return (
    <div>
      <Card
        title={
          currentSessionDbId
            ? `Modification Session (ID: ${currentSessionDbId}) - Statut: ${
                editingSessionData?.status || "N/A"
              }`
            : "Nouvelle session"
        }
        className="mb-6"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
          <Select
            ...
            onChange={(e) =>
              setSelectedReferential(e.target.value as CACESReferential | "")
            }
            placeholder="Sélectionner un référentiel"
            required
            disabled={
              !!editingSessionData?.questionMappings ||
        ...
        <div className="mt-6">
          ...
          <Input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              setTemplateFile(e.target.files ? e.target.files[0] : null)
            }
          ...
          {templateFile && (
            <p className="mt-1 text-xs text-green-600">
              Fichier: {templateFile.name}
            </p>
          )}
          {editingSessionData?.donneesOrs && !templateFile && (
            <p className="mt-1 text-xs text-blue-600">
              Un .ors a déjà été généré. Pour le régénérer, sélectionnez un
              nouveau modèle.
            </p>
          )}
        </div>
        {currentSessionDbId &&
          editingSessionData?.selectionBlocs &&
          editingSessionData.selectionBlocs.length > 0 && (
            <div className="mt-6 p-4 border border-gray-200 rounded-lg bg-gray-50 mb-6">
              <h4 className="text-md font-semibold text-gray-700 mb-2">
                Blocs thématiques sélectionnés:
              </h4>
              <ul className="list-disc list-inside pl-2 space-y-1">
                {editingSessionData.selectionBlocs.map((sb) => (
                  <li
                    key={`${sb.theme}-${sb.blockId}`}
                    className="text-sm text-gray-600"
                  >
                    <span className="font-medium">{sb.theme}:</span> Bloc{" "}
                    {sb.blockId}
                  </li>
                ))}
              </ul>
          ...
          <div className="space-y-4">
            ...
            {editingSessionData?.status === "completed" && (
              <p className="text-sm text-yellow-700 bg-yellow-100 p-2 rounded-md">
                Résultats déjà importés (session terminée).
              </p>
            )}
            {editingSessionData &&
              !editingSessionData.questionMappings &&
              editingSessionData.status !== "completed" && (
                <p className="text-sm text-yellow-700 bg-yellow-100 p-2 rounded-md">
                  Générez d'abord le .ors pour cette session.
                </p>
              )}
            <p className="text-xs text-gray-500">
              Importez le fichier .ors après le vote.
            </p>
            {importSummary && (
              <div
                className={`mt-4 p-3 rounded-md text-sm ${
                  importSummary.toLowerCase().includes("erreur") ||
                  importSummary.toLowerCase().includes("échoué") ||
                  importSummary.toLowerCase().includes("impossible")
      ...
      <Card title="Participants" className="mb-6">
        <div className="mb-4 flex items-center justify-between">
          <p className="text-sm text-gray-500">
            Gérez la liste des participants.
          </p>
                ...
                participants.map((participant) => (
                  <tr key={participant.id}>
                    ...
                    <td className="px-4 py-2 whitespace-nowrap text-center">
                      {participant.reussite === true && (
                        <Badge variant="success">Réussi</Badge>
                      )}
                      {participant.reussite === false && (
        ...
        {participants.length > 0 && (
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>Attribution boîtiers :</strong> Le numéro de boîtier est
              utilisé pour identifier les participants lors de l'import des
              résultats.
            </p>
          </div>
        )}
