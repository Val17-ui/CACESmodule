...
import {
  ...
  Download,
  AlertTriangle,
} from "lucide-react"; // Ajout AlertTriangle pour erreurs
...
const SessionsList: React.FC<SessionsListProps> = ({
  ...
  const handleDownloadOrs = (session: DBSession) => {
    if (session.donneesOrs instanceof Blob) {
      const orsFileName = `Session_${session.nomSession.replace(
        /[^a-z0-9]/gi,
        "_"
      )}_${session.id || "id"}.ors`;
      ...
      alert(
        "Fichier .ors non disponible ou format incorrect pour le téléchargement."
      );
      console.warn(
        "Tentative de téléchargement d'un .ors non-Blob:",
        session.donneesOrs
      );
    }
  };
            ...
            {sessions.length === 0 ? (
              ...
              sessions.map((session) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Badge variant="info">
                      {(session.referentiel as CACESReferential) || "N/A"}
                    </Badge>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    {session.donneesOrs instanceof Blob ? (
                      <Badge
                        ...
                        title={`Taille: ${Math.round(
                          session.donneesOrs.size / 1024
                        )} Ko`}
                      >
                        Oui
                      </Badge>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                      <Button
                        ...
                        onClick={() =>
                          session.id && onManageSession(session.id)
                        }
                      ...
                      {(session.status === "planned" || !session.status) &&
                        session.donneesOrs instanceof Blob && (
                          <Button
                            ...
                            icon={<Play size={16} />}
                            onClick={() =>
                              session.id && onStartExam(session.id)
                            }
