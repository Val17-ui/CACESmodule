...
const QuestionnaireForm: React.FC = () => {
  ...
  const handleReferentialChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    logger.info(`Référentiel sélectionné: ${e.target.value}`);
    setShowValidationWarning(e.target.value === 'R482');
  };
  ...
  const totalQuestions = Object.values(themeDistribution).reduce((sum, count) => sum + count, 0);
      ...
      <Card title="Informations générales" className="mb-6">
        ...
        <div className="mt-4 flex items-center space-x-2">
          ...
          <label htmlFor="randomize" className="text-sm text-gray-700">
            Mode aléatoire (sélection automatique des questions)
          </label>
        </div>
      ...
      <Card title="Questions" className="mb-6">
        <div className="mb-4 flex justify-between items-center">
          <h4 className="text-sm font-medium text-gray-700">
            Questions sélectionnées ({totalQuestions})
          </h4>
          {isRandomized && (
            <Button
              variant="outline"
              ...
              onClick={() => logger.info('Régénération aléatoire des questions')}
            >
