...
exports.transformQuestionsForVal17Generator = transformQuestionsForVal17Generator;
...
const val17PptxGenerator_1 = require("./val17PptxGenerator");
function generateOmbeaSessionXml(sessionInfo, participants, _questionMappings // Utiliser QuestionMapping ici, même si non utilisé dans ce XML particulier
) {
    ...
    participants.forEach((p, index) => {
        ...
        xml += `          <rl:Device>${esc((_a = p.assignedGlobalDeviceId) === null || _a === void 0 ? void 0 : _a.toString())}</rl:Device>\n`;
        xml += `        </rl:Devices>\n`;
...
function alertAlreadyShown(_error) {
    // In a real app, this would check if a user-facing error for this operation
    // has already been displayed.
    return false;
}
let tempImageUrls = [];
function transformQuestionsForVal17Generator(storedQuestions) {
    tempImageUrls.forEach(url => {
        try {
            URL.revokeObjectURL(url);
        }
        catch (e) {
            console.warn("Failed to revoke URL for transformQuestionsForVal17Generator:", url, e);
        }
    });
    tempImageUrls = [];
    return storedQuestions.map((sq) => {
        ...
        return {
            dbQuestionId: sq.id, // Assurer que l'ID est bien passé
            question: sq.text,
            options: sq.options,
...
function generatePresentation(sessionInfo, _participants, storedQuestions, templateFile, adminSettings) {
    return __awaiter(this, void 0, void 0, function* () {
        let templateBuffer;
        if (templateFile instanceof File) {
            templateBuffer = yield templateFile.arrayBuffer();
        }
        else {
        ...
        try {
            ...
            const participantsForGenerator = _participants.map(p => {
                var _a;
                return ({
                    idBoitier: (_a = p.assignedGlobalDeviceId) === null || _a === void 0 ? void 0 : _a.toString(),
                    nom: p.nom,
                    prenom: p.prenom,
                    identificationCode: p.identificationCode
                });
            });
            const generatedData = yield (0, val17PptxGenerator_1.generatePPTXVal17)(templateBuffer, transformedQuestions, generationOptions, val17SessionInfo, participantsForGenerator);
            if (generatedData && generatedData.pptxBlob && generatedData.questionMappings && generatedData.preExistingQuestionSlideGuids) {
                const orSessionXmlContent = generateOmbeaSessionXml(val17SessionInfo, _participants, generatedData.questionMappings);
                const outputOrsZip = new jszip_1.default();
                ...
                const orsBlob = yield outputOrsZip.generateAsync({ type: 'blob', mimeType: 'application/octet-stream' });
                return {
                    orsBlob: orsBlob,
