...
exports.handleGeneratePPTXFromVal17Tool = void 0;
exports.generatePPTXVal17 = generatePPTXVal17;
...
function countExistingSlides(zip) {
    var _a;
    let count = 0;
    (_a = zip.folder("ppt/slides")) === null || _a === void 0 ? void 0 : _a.forEach((relativePath) => {
...
function downloadImageFromCloudWithDimensions(url) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const dimensions = yield getImageDimensions(blob);
            return {
                data: arrayBuffer,
                extension,
...
function updateContentTypesForImages(content, imageExtensions) {
    let updated = content;
...
function findNextAvailableSlideLayoutId(zip) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const masterRelsContent = yield masterRelsFile.async("string");
...
function ensureOmbeaSlideLayoutExists(zip) {
    return __awaiter(this, void 0, void 0, function* () {
        const { layoutId, layoutFileName, rId } = yield findNextAvailableSlideLayoutId(zip);
        ...
        yield updateSlideMasterRelsForNewLayout(zip, layoutFileName, rId);
        yield updateSlideMasterForNewLayout(zip, layoutId, rId);
        yield updateContentTypesForNewLayout(zip, layoutFileName);
        return { layoutFileName: layoutFileName, layoutRId: rId };
    });
}
function updateSlideMasterRelsForNewLayout(zip, layoutFileName, rId) {
    return __awaiter(this, void 0, void 0, function* () {
        const masterRelsFile = zip.file("ppt/slideMasters/_rels/slideMaster1.xml.rels");
        if (masterRelsFile) {
            let content = yield masterRelsFile.async("string");
...
function updateSlideMasterForNewLayout(zip, layoutId, rId) {
    return __awaiter(this, void 0, void 0, function* () {
        const masterFile = zip.file("ppt/slideMasters/slideMaster1.xml");
        if (masterFile) {
            let content = yield masterFile.async("string");
...
function updateContentTypesForNewLayout(zip, layoutFileName) {
    return __awaiter(this, void 0, void 0, function* () {
        const contentTypesFile = zip.file("[Content_Types].xml");
        if (contentTypesFile) {
            let content = yield contentTypesFile.async("string");
...
function createIntroTitleSlideXml(sessionInfo, slideNumber) {
    ...
    <p:txBody>
      <a:bodyPr/><a:lstStyle/>
      <a:p><a:r><a:rPr lang="fr-FR"/><a:t>${escapeXml(sessionInfo.title)}</a:t></a:r></a:p>
    </p:txBody>
  </p:sp>`;
    ...
    <p:txBody>
      <a:bodyPr/><a:lstStyle/>
      <a:p><a:r><a:rPr lang="fr-FR"/><a:t>${escapeXml(sessionInfo.date)}</a:t></a:r></a:p>
    </p:txBody>
  </p:sp>`
        : "";
...
function generateTableRowsXml(participants, rowHeightEMU = 370840) {
    let tableRowsXml = "";
...
function generateTableGraphicFrame(participants, baseSpId) {
    const hasOrganizationData = participants.some(p => p.organization && p.organization.trim() !== "");
...
function createIntroParticipantsSlideXml(participants, slideNumber, layoutPptxFilePath, layoutXmlAsSlideBase, layoutGraphicFrameTarget, layoutTblPr, layoutTblGrid) {
    ...
    if (layoutXmlAsSlideBase && layoutGraphicFrameTarget && layoutTblPr && layoutTblGrid) {
        ...
        const graphicFrameWithNewTable = layoutGraphicFrameTarget.replace(/<a:tbl>[\s\S]*?<\/a:tbl>/, newFullTblXml);
...
//   </p:sld>`;
// }
function createSlideXml(question, options, slideNumber, duration = 30, imageDimensions, ombeaConfig) {
    ...
    const countdownDisplayText = // prefer-const
     (ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.pollTimeLimit) !== undefined
        ? ombeaConfig.pollTimeLimit
        : duration;
    let bulletTypeForXml = "arabicPeriod";
    if (ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.answersBulletStyle) {
        const styleMap = {
    ...
    xmlContent += `<p:sp><p:nvSpPr><p:cNvPr id="${bodyId}" name="Espace réservé du texte ${slideNumber}"/><p:cNvSpPr><a:spLocks noGrp="1"/></p:cNvSpPr><p:nvPr><p:ph type="body" idx="1"/><p:custDataLst><p:tags r:id="rId3"/></p:custDataLst></p:nvPr></p:nvSpPr><p:spPr><a:xfrm><a:off x="457200" y="1600200"/><a:ext cx="4572000" cy="4525963"/></a:xfrm></p:spPr><p:txBody><a:bodyPr/>${listStyleXml}${options
        .map((option) => `<a:p><a:pPr><a:buFont typeface="+mj-lt"/><a:buAutoNum type="${bulletTypeForXml}"/></a:pPr><a:r><a:rPr lang="fr-FR" dirty="0"/><a:t>${escapeXml(option)}</a:t></a:r></a:p>`)
        .join("")}</p:txBody></p:sp>`;
    if (Number(countdownDisplayText) > 0) {
        // Nouvelles coordonnées X=7380000 (20.5cm), Y=3722400 (10.34cm)
        xmlContent += `<p:sp><p:nvSpPr><p:cNvPr id="${countdownId}" name="OMBEA Countdown ${slideNumber}"/><p:cNvSpPr txBox="1"/><p:nvPr><p:custDataLst><p:tags r:id="rId4"/></p:custDataLst></p:nvPr></p:nvSpPr><p:spPr><a:xfrm><a:off x="7380000" y="3722400"/><a:ext cx="1524000" cy="769441"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:noFill/></p:spPr><p:txBody><a:bodyPr vert="horz" rtlCol="0" anchor="ctr" anchorCtr="1"><a:spAutoFit/></a:bodyPr><a:lstStyle/><a:p><a:r><a:rPr lang="fr-FR" sz="4400" smtClean="0"/><a:t>${String(countdownDisplayText)}</a:t></a:r><a:endParaRPr lang="fr-FR" sz="4400"/></a:p></p:txBody></p:sp>`;
    }
...
function calculateBaseTagNumber(slideNumberInBatch, tagOffset = 0) {
    return tagOffset + 1 + (slideNumberInBatch - 1) * 4;
}
function findHighestExistingTagNumber(zip) {
    let maxTagNumber = 0;
...
function findLayoutByCSldName(zip, targetName, layoutType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const fileEntry of files) {
            if (fileEntry) {
                try {
                    const content = yield fileEntry.async("string");
...
function ensureTagContinuity(zip, startingTag, endingTag) {
    const warnings = [];
...
function createSlideTagFiles(questionIndexInBatch, options, correctAnswerIndex, duration, ombeaConfig, tagOffset = 0) {
    ...
    const points = options // prefer-const
        .map((_, index) => correctAnswerIndex !== undefined && index === correctAnswerIndex
        ? "1.00"
        : "0.00")
        .join(",");
    const tags = [];
    tags.push({
        ...
        content: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><p:tagLst xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:tag name="OR_SLIDE_GUID" val="${slideGuid}"/><p:tag name="OR_OFFICE_MAJOR_VERSION" val="14"/><p:tag name="OR_POLL_START_MODE" val="${(ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.pollStartMode) || "Automatic"}"/><p:tag name="OR_CHART_VALUE_LABEL_FORMAT" val="${(ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.chartValueLabelFormat) || "Response_Count"}"/><p:tag name="OR_CHART_RESPONSE_DENOMINATOR" val="Responses"/><p:tag name="OR_CHART_FIXED_RESPONSE_DENOMINATOR" val="100"/><p:tag name="OR_CHART_COLOR_MODE" val="Color_Scheme"/><p:tag name="OR_CHART_APPLY_OMBEA_TEMPLATE" val="True"/><p:tag name="OR_POLL_DEFAULT_ANSWER_OPTION" val="None"/><p:tag name="OR_SLIDE_TYPE" val="OR_QUESTION_SLIDE"/><p:tag name="OR_ANSWERS_BULLET_STYLE" val="${(ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.answersBulletStyle) || "ppBulletArabicPeriod"}"/><p:tag name="OR_POLL_FLOW" val="Automatic"/><p:tag name="OR_CHART_DISPLAY_MODE" val="Automatic"/><p:tag name="OR_POLL_TIME_LIMIT" val="${(ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.pollTimeLimit) !== undefined
            ? ombeaConfig.pollTimeLimit
            : duration}"/><p:tag name="OR_POLL_COUNTDOWN_START_MODE" val="${(ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.pollCountdownStartMode) || "Automatic"}"/><p:tag name="OR_POLL_MULTIPLE_RESPONSES" val="${(ombeaConfig === null || ombeaConfig === void 0 ? void 0 : ombeaConfig.pollMultipleResponse) !== undefined
            ? ombeaConfig.pollMultipleResponse
            : "1"}"/><p:tag name="OR_POLL_DUPLICATES_ALLOWED" val="False"/><p:tag name="OR_CATEGORIZING" val="False"/><p:tag name="OR_PRIORITY_RANKING" val="False"/><p:tag name="OR_IS_POLLED" val="False"/></p:tagLst>`,
    });
    tags.push({
...
function extractExistingRIds(relsContent) {
    const mappings = [];
...
function getNextAvailableRId(existingRIds) {
    let maxId = 0;
...
function updatePresentationRelsWithMappings(originalContent, initialExistingSlideCount, introSlideDetails, newOmbeaQuestionCount) {
    const existingRels = extractExistingRIds(originalContent);
    const finalRelsOutput = [];
    const slideRIdMappings = [];
    const oldToNewRIdMap = {};
    let rIdCounter = 1;
    ...
    if (originalSlideMaster) {
        finalRelsOutput.push(Object.assign(Object.assign({}, originalSlideMaster), { rId: "rId1" }));
    ...
    for (let i = 0; i < initialExistingSlideCount; i++) {
        ...
        finalRelsOutput.push({
            ...
            type: slideType,
            target: slideTarget,
            originalRId: originalRel === null || originalRel === void 0 ? void 0 : originalRel.rId,
        });
        slideRIdMappings.push({ slideNumber: finalSlideOrderIndex, rId: newRId });
        if (originalRel)
            oldToNewRIdMap[originalRel.rId] = newRId;
    }
    ...
    existingRels.forEach((origRel) => {
        if (origRel.type !== slideMasterType && origRel.type !== slideType) {
            if (!oldToNewRIdMap[origRel.rId]) {
                const newRId = `rId${rIdCounter++}`;
                finalRelsOutput.push(Object.assign(Object.assign({}, origRel), { rId: newRId }));
                oldToNewRIdMap[origRel.rId] = newRId;
            }
            else {
                finalRelsOutput.push(Object.assign(Object.assign({}, origRel), { rId: oldToNewRIdMap[origRel.rId] }));
            }
        }
    });
...
function rebuildPresentationXml(zip, slideRIdMappings, slideSizeAttrs, oldToNewRIdMap) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let content = yield presentationFile.async("string");
        ...
        if (slideSizeAttrs) {
            ...
            if (sldSzRegex.test(content)) {
                ...
                if (insertPoint !== -1) {
                    ...
                    if (insertPoint !== -1) {
                        content = `${content.slice(0, insertPoint)}${newSldSzTag}\n  ${content.slice(insertPoint)}`;
                    }
                    else {
                        ...
                        if (insertPoint !== -1) {
                            ...
                            console.warn("Could not find a suitable place to insert <p:sldSz> in presentation.xml.");
                        }
                    }
                }
            }
...
function updateContentTypesComplete(originalContent, introSlideDetails, newOmbeaQuestionCount, totalSlidesInFinalPptx, ombeaQuestionLayoutFileName, totalTagsUsed) {
    ...
    if (!updatedContent.includes(`PartName="${ombeaLayoutPartName}"`)) {
        ...
        if (lastLayoutIdx > -1)
            ...
            insertPt = updatedContent.lastIndexOf("</Types>");
        if (insertPt > -1) {
    ...
    const slidesBeforeOmbeaQuestions = totalSlidesInFinalPptx - newOmbeaQuestionCount;
    for (let i = 0; i < newOmbeaQuestionCount; i++) {
...
function calculateAppXmlMetadata(totalFinalSlides, newOmbeaQuestions) {
    let totalWords = 0;
...
function updateAppXml(zip, metadata) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let content = yield appFile.async("string");
...
function updateSimpleFields(content, metadata) {
    let updated = content;
    ...
    const existingParagraphs = paragraphsMatch && paragraphsMatch[1]
    ...
    if (!updated.includes("<Company")) {
        ...
        if (insertPoint > -1)
            insertPoint += insertAfter.length;
        else
            insertPoint = updated.indexOf("</Properties>");
        if (insertPoint > -1) {
            const companyTag = "\n  <Company/>";
            updated =
...
function updateHeadingPairsAndTitles(content, newOmbeaSlideTitles) {
    let updated = content;
    ...
    if (titlesOfPartsEndIndex !== -1) {
        ...
        newOmbeaSlideTitles.forEach((title) => {
            titlesXmlToAdd += `\n      <vt:lpstr>${escapeXml(title.substring(0, 250))}</vt:lpstr>`;
        });
        updated =
            updated.slice(0, titlesOfPartsEndIndex) +
                titlesXmlToAdd +
                updated.slice(titlesOfPartsEndIndex);
        updated = updated.replace(/<TitlesOfParts>\s*<vt:vector size="(\d+)"/, (_match, p1) => {
...
function buildHeadingPairs(nonSlideTitles, allSlideTitles) {
    const pairs = [];
    const fontCount = nonSlideTitles.filter((t) => t.includes("Arial") ||
        t.includes("Calibri") ||
        t.includes("Font") ||
        t.includes("Police")).length;
    if (fontCount > 0) {
...
function buildTitlesOfParts(fonts, themes, existingSlideTitles, newSlideTitles) {
    const allTitles = [];
    ...
    newSlideTitles.forEach((title) => {
        const truncatedTitle = title.length > 250 ? title.substring(0, 247) + "..." : title;
...
function createNewAppXml(zip, metadata) {
...
function updateCoreXml(zip, newQuestionCount) {
    return __awaiter(this, void 0, void 0, function* () {
        const coreFile = zip.file("docProps/core.xml");
        if (coreFile) {
            let content = yield coreFile.async("string");
...
function getLayoutXml(zip, layoutFileName) {
    return __awaiter(this, void 0, void 0, function* () {
...
function generatePPTXVal17(templateFile_1, questions_1) {
    return __awaiter(this, arguments, void 0, function* (templateFile, questions, options = {}, sessionInfo, participants) {
        var _a, _b, _c;
        try {
            ...
            if (slideFilesFolder) {
                ...
                slideFilesFolder.forEach((relativePath, slideFileEntry) => {
                    if (relativePath.startsWith("slide") && relativePath.endsWith(".xml") && !slideFileEntry.dir) {
                        ...
                        if (slideNumberMatch && slideNumberMatch[1]) {
                            ...
                            if (relsFile) {
                                const promise = relsFile.async("string").then((relsContent) => __awaiter(this, void 0, void 0, function* () {
                                    ...
                                    while ((relMatch = tagRelationshipRegex.exec(relsContent)) !== null) {
                                        ...
                                        if (tagFile) {
                                            try {
                                                const tagContent = yield tagFile.async("string");
                ...
                yield Promise.all(slideProcessingPromises);
            ...
            let slideSizeAttrs = null;
            const presentationXmlFileFromTemplate = templateZip.file("ppt/presentation.xml");
            if (presentationXmlFileFromTemplate) {
                const presentationXmlContent = yield presentationXmlFileFromTemplate.async("string");
                const sldSzMatch = presentationXmlContent.match(/<p:sldSz\s+cx="(\d+)"\s+cy="(\d+)"(?:\s+type="(\w+)")?/);
                if (sldSzMatch) {
                    ...
                    console.warn("<p:sldSz> non trouvé dans le presentation.xml du modèle.");
                }
            }
            else {
            ...
            if (sessionInfo && ((_a = options.introSlideLayouts) === null || _a === void 0 ? void 0 : _a.titleLayoutName)) {
                const targetTitleLayoutName = options.introSlideLayouts.titleLayoutName;
                const actualTitleLayoutPath = yield findLayoutByCSldName(outputZip, targetTitleLayoutName, "title");
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="${layoutRIdInSlide}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/${titleLayoutBaseName}"/>
            ...
            if (participants && participants.length > 0 && ((_b = options.introSlideLayouts) === null || _b === void 0 ? void 0 : _b.participantsLayoutName)) {
                const targetParticipantsLayoutName = options.introSlideLayouts.participantsLayoutName;
                const actualParticipantsLayoutPath = yield findLayoutByCSldName(outputZip, targetParticipantsLayoutName, "participants");
                if (actualParticipantsLayoutPath) {
                    let layoutTblPrXml = null;
                    let layoutTblGridXml = null;
                    let layoutGraphicFrameXml = null;
                    const layoutFileXmlContent = yield getLayoutXml(outputZip, actualParticipantsLayoutPath);
                    ...
                    const currentIntroSlideNumber = initialExistingSlideCount + introSlidesAddedCount + 1;
                    const participantsSlideXml = createIntroParticipantsSlideXml(participants, currentIntroSlideNumber, actualParticipantsLayoutPath, layoutFileXmlContent, layoutGraphicFrameXml, layoutTblPrXml, layoutTblGridXml);
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="${layoutRIdInSlide}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/${participantsLayoutBaseName}"/>
            ...
            const effectiveExistingSlideCount = initialExistingSlideCount + introSlidesAddedCount;
            const ombeaLayout = yield ensureOmbeaSlideLayoutExists(outputZip);
            ...
            const questionMappingsInternal = [];
            if (questions.some((q) => q.imageUrl)) {
                const imagePromises = questions.map((question, index) => __awaiter(this, void 0, void 0, function* () {
                    if (question.imageUrl) {
                        try {
                            const imageData = yield downloadImageFromCloudWithDimensions(question.imageUrl);
                            if (imageData) {
                                const absoluteSlideNumberForImage = effectiveExistingSlideCount + index + 1;
                            ...
                            console.error(`Erreur téléchargement image pour question ${index + 1} (${question.imageUrl}):`, error);
                        }
                    }
                    return null;
                }));
                ...
                imageResults.forEach((result) => {
                    ...
                    if (result && result.image) {
                        ...
                        (_a = outputZip
                            .folder("ppt/media")) === null || _a === void 0 ? void 0 : _a.file(result.image.fileName, result.image.data);
                    }
                });
            }
            for (let i = 0; i < questions.length; i++) {
                ...
                const duration = questionData.points ||
                    ((_c = options.ombeaConfig) === null || _c === void 0 ? void 0 : _c.pollTimeLimit) ||
                    options.defaultDuration ||
                    30;
                const downloadedImage = downloadedImages.get(absoluteSlideNumber);
                const slideXml = createSlideXml(questionData.question, questionData.options, absoluteSlideNumber, duration, downloadedImage === null || downloadedImage === void 0 ? void 0 : downloadedImage.dimensions, options.ombeaConfig);
                ...
                tags.forEach((tag) => {
                    ...
                    if (tag.tagNumber > maxTagNumberUsed)
                        maxTagNumberUsed = tag.tagNumber;
                });
            ...
            if (contentTypesFile) {
                let contentTypesContent = yield contentTypesFile.async("string");
                if (imageExtensions.size > 0)
                    contentTypesContent = updateContentTypesForImages(contentTypesContent, imageExtensions);
                contentTypesContent = updateContentTypesComplete(contentTypesContent, newIntroSlideDetails.map((d) => ({
                    slideNumber: d.slideNumber,
                    layoutFileName: d.layoutFileName,
                })), questions.length, totalFinalSlideCount, ombeaLayoutFileName, maxTagNumberUsed);
            ...
            if (presentationRelsFile) {
                const presentationRelsContent = yield presentationRelsFile.async("string");
                const { updatedContent: updatedPresentationRels, slideRIdMappings, oldToNewRIdMap, } = updatePresentationRelsWithMappings(presentationRelsContent, initialExistingSlideCount, newIntroSlideDetails, questions.length);
                outputZip.file("ppt/_rels/presentation.xml.rels", updatedPresentationRels);
                yield rebuildPresentationXml(outputZip, slideRIdMappings, slideSizeAttrs, oldToNewRIdMap);
            }
            yield updateCoreXml(outputZip, questions.length);
            const appMetadata = calculateAppXmlMetadata(totalFinalSlideCount, questions);
            yield updateAppXml(outputZip, appMetadata);
            const outputBlob = yield outputZip.generateAsync({
                type: "blob",
...
function testConsistency(templateFile, questions) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < 1; i++) {
            ...
            try {
                const templateCopy = new File([yield templateFile.arrayBuffer()], templateFile.name, { type: templateFile.type });
                const result = yield generatePPTXVal17(templateCopy, questions, { fileName: `Test_Coherence_${i + 1}.pptx` }, undefined, undefined);
...
const handleGeneratePPTXFromVal17Tool = (templateFile, questions) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const result = yield generatePPTXVal17(templateFile, questions, { fileName: "Quiz_OMBEA_Interactif_Val17.pptx" }, undefined, undefined);
        if (result && result.pptxBlob) {
            console.log("handleGeneratePPTXFromVal17Tool: PPTX Blob généré, sauvegarde...");
            (0, file_saver_1.saveAs)(result.pptxBlob, "Quiz_OMBEA_Interactif_Val17_Tool.pptx");
        }
        else {
...
exports.handleGeneratePPTXFromVal17Tool = handleGeneratePPTXFromVal17Tool;
