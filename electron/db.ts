...
log(`[DB SETUP] Database path determined as: ${dbPath}`);
...
export function initializeDatabase() {
  ...
  log('[DB SETUP] Initializing database...');
  db = new Database(dbPath);
  log(`[DB SETUP] SQLite database connection established.`);

  // Activer les clés étrangères
  try {
    getDb().pragma('foreign_keys = ON');
    log("[DB SETUP] Foreign key support enabled.");
  } catch (error) {
    log(`[DB SETUP] Failed to enable foreign keys: ${error}`);
  }
  ...
  try {
    ...
    log(`[DB SETUP] FATAL: Failed to create/verify database schema. Application might not work correctly. ${error}`);
...
const createSchema = () => {
  log("[DB SCHEMA] Attempting to create/verify schema...");
  ...
  const transaction = getDb().transaction(() => {
    for (const stmt of DDL_STATEMENTS) {
      try {
        ...
        log(`[DB SCHEMA] Failed to execute DDL: ${stmt.substring(0,60)}... ${error}`);
  ...
  try {
    transaction();
    log("[DB SCHEMA] Database schema created/verified successfully.");
    ...
    try {
        ...
        if (hasDonneesOrs && !hasOrsFilePath) {
            ...
            log("[DB MIGRATION] Renamed column 'donneesOrs' to 'orsFilePath'.");
        }
    } catch (error) {
        // This might fail for other reasons, log it but proceed.
        log(`[DB MIGRATION] Error during rename check/operation for 'donneesOrs': ${error}`);
    }
    ...
    for (const column of columnsToAdd) {
        if (!existingColumns.includes(column.name)) {
            try {
                ...
                log(`[DB MIGRATION] Added '${column.name}' column to 'sessions' table.`);
            } catch (error: any) {
                // Catching errors here just in case, though the check should prevent duplicates.
                log(`[DB MIGRATION] Error adding '${column.name}' column: ${error}`);
            }
        }
    }

  } catch(error) {
    log(`[DB SCHEMA] Transaction failed during schema creation. No changes were applied. ${error}`);
...
async function asyncDbRun<T>(fn: () => T): Promise<T> {
  try {
    ...
    log(`[ASYNC DB RUNNER] SQLite operation failed: ${error}`);
...
const addQuestion = async (question: Omit<QuestionWithId, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error adding question: ${error}`);
      throw error;
    }
  });
};
...
const getAllQuestions = async (): Promise<QuestionWithId[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error getting all questions: ${error}`);
      throw error;
    }
  });
};
...
const getQuestionById = async (id: number): Promise<QuestionWithId | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error getting question by id ${id}: ${error}`);
      throw error;
    }
  });
}
...
const getQuestionsByIds = async (ids: number[]): Promise<QuestionWithId[]> => {
  ...
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error getting questions by ids: ${error}`);
      throw error;
    }
  });
}
...
const updateQuestion = async (id: number, updates: Partial<Omit<QuestionWithId, 'id'>>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error updating question ${id}: ${error}`);
      throw error;
    }
  });
}
...
const deleteQuestion = async (id: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error deleting question ${id}: ${error}`);
      throw error;
    }
  });
}
...
const getQuestionsByBlocId = async (blocId: number): Promise<QuestionWithId[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error getting questions by blocId ${blocId}: ${error}`);
      throw error;
    }
  });
}
...
const getQuestionsForSessionBlocks = async (selectedBlocIds?: number[]): Promise<QuestionWithId[]> => {
  if (!selectedBlocIds || selectedBlocIds.length === 0) {
    ...
    return asyncDbRun(() => {
      try {
        ...
        log(`[DB Questions] Error getting questions with no blocId: ${error}`);
        throw error;
      }
    });
  }
  ...
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Questions] Error getting questions for session blocks: ${error}`);
      throw error;
    }
  });
}
...
const getAdminSetting = async (key: string): Promise<any> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB AdminSettings] Error getting setting ${key}: ${error}`);
      throw error;
    }
  });
};
...
const setAdminSetting = async (key: string, value: any): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB AdminSettings] Error setting setting ${key}: ${error}`);
      throw error;
    }
  });
};
...
const getAllAdminSettings = async (): Promise<{ key: string; value: any }[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB AdminSettings] Error getting all settings: ${error}`);
      throw error;
    }
  });
};
...
const getGlobalPptxTemplate = async (): Promise<File | null> => {
  log("[DB AdminSettings] getGlobalPptxTemplate: Returning null. Logic to fetch/construct File object from path/blob is outside direct DB scope for now.");
  // Potentiellement:
  ...
  for (const field of JSON_SESSION_FIELDS) {
    if (session[field] && typeof session[field] === 'string') {
      try {
        ...
        log(`[DB Sessions] Failed to parse JSON field ${field} for session id ${row.id}: ${e}`);
...
const addSession = async (session: Omit<Session, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Sessions] Error adding session: ${error}`);
      throw error;
    }
  });
};
...
const getAllSessions = async (): Promise<Session[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Sessions] Error getting all sessions: ${error}`);
      throw error;
    }
  });
};
...
const getSessionById = async (id: number): Promise<Session | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Sessions] Error getting session by id ${id}: ${error}`);
      throw error;
    }
  });
};
...
const updateSession = async (id: number, updates: Partial<Omit<Session, 'id'>>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Sessions] Error updating session ${id}: ${error}`);
      throw error;
    }
  });
};
...
const deleteSession = async (id: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Sessions] Error deleting session ${id}: ${error}`);
      throw error;
    }
  });
};
...
const addSessionResult = async (result: Omit<SessionResult, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionResults] Error adding session result: ${error}`);
      throw error;
    }
  });
};
...
const addBulkSessionResults = async (results: Omit<SessionResult, 'id'>[]): Promise<(number | undefined)[] | undefined> => {
  ...
  return asyncDbRun(() => {
    ...
    const transaction = getDb().transaction((items: Omit<SessionResult, 'id'>[]) => {
      for (const result of items) {
        try {
          ...
          log(`[DB SessionResults] Error in bulk adding session result for item: ${result}, ${error}`);
    ...
    try {
      ...
      log(`[DB SessionResults] Bulk add transaction failed.`);
      return undefined;
    }
  });
};
...
const getAllResults = async (): Promise<SessionResult[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionResults] Error getting all results: ${error}`);
      throw error;
    }
  });
};
...
const getResultsForSession = async (sessionId: number): Promise<SessionResult[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionResults] Error getting results for session ${sessionId}: ${error}`);
      throw error;
    }
  });
};
...
const getResultBySessionAndQuestion = async (sessionId: number, questionId: number, participantIdBoitier: string): Promise<SessionResult | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionResults] Error getting result for session ${sessionId}, question ${questionId}, boitier ${participantIdBoitier}: ${error}`);
      throw error;
    }
  });
};
...
const updateSessionResult = async (id: number, updates: Partial<Omit<SessionResult, 'id'>>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionResults] Error updating session result ${id}: ${error}`);
      throw error;
    }
  });
};
...
const deleteResultsForSession = async (sessionId: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionResults] Error deleting results for session ${sessionId}: ${error}`);
      throw error;
    }
  });
};
...
const addVotingDevice = async (device: Omit<VotingDevice, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB VotingDevices] Error adding voting device: ${error}`);
...
const getAllVotingDevices = async (): Promise<VotingDevice[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB VotingDevices] getAllVotingDevices returned: ${devices.length} devices`);
      return devices;
    } catch (error) {
      log(`[DB VotingDevices] Error getting all voting devices: ${error}`);
      throw error;
    }
  });
};
...
const updateVotingDevice = async (id: number, updates: Partial<Omit<VotingDevice, 'id'>>): Promise<number> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB VotingDevices] Error updating voting device ${id}: ${error}`);
...
const deleteVotingDevice = async (id: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB VotingDevices] Error deleting voting device ${id}: ${error}`);
      throw error;
    }
  });
};
...
const bulkAddVotingDevices = async (devices: Omit<VotingDevice, 'id'>[]): Promise<void> => {
  ...
  return asyncDbRun(() => {
    ...
    const transaction = getDb().transaction((items: Omit<VotingDevice, 'id'>[]) => {
      for (const device of items) {
        try {
          ...
          log(`[DB VotingDevices] Error in bulk adding voting device for item: ${device}, ${error}`);
    ...
    try {
      ...
      log(`[DB VotingDevices] Bulk add transaction failed overall.`);
...
const addTrainer = async (trainer: Omit<Trainer, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Trainers] Error adding trainer: ${error}`);
      throw error;
    }
  });
};
...
const getAllTrainers = async (): Promise<Trainer[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Trainers] Error getting all trainers: ${error}`);
      throw error;
    }
  });
};
...
const getTrainerById = async (id: number): Promise<Trainer | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Trainers] Error getting trainer by id ${id}: ${error}`);
      throw error;
    }
  });
};
...
const updateTrainer = async (id: number, updates: Partial<Omit<Trainer, 'id'>>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Trainers] Error updating trainer ${id}: ${error}`);
      throw error;
    }
  });
};
...
const deleteTrainer = async (id: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Trainers] Error deleting trainer ${id}: ${error}`);
      throw error;
    }
  });
};
  ...
  return asyncDbRun(() => {
    const transaction = getDb().transaction(() => {
      try {
        ...
        if (result.changes === 0) {
          log(`[DB Trainers] setDefaultTrainer: Trainer with id ${id} not found or no change made.`);
        }
      } catch (error) {
        log(`[DB Trainers] Error setting default trainer ${id}: ${error}`);
...
const getDefaultTrainer = async (): Promise<Trainer | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Trainers] Error getting default trainer: ${error}`);
      throw error;
    }
  });
};
...
const addSessionQuestion = async (sq: Omit<SessionQuestion, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionQuestions] Error adding session question: ${error}`);
      throw error;
    }
  });
};
...
const addBulkSessionQuestions = async (questions: Omit<SessionQuestion, 'id'>[]): Promise<(number | undefined)[] | undefined> => {
  ...
  return asyncDbRun(() => {
    ...
    const transaction = getDb().transaction((items: Omit<SessionQuestion, 'id'>[]) => {
      for (const question of items) {
        try {
          ...
          log(`[DB SessionQuestions] Error in bulk adding session question for item: ${question}, ${error}`);
    ...
    try {
      ...
      log(`[DB SessionQuestions] Bulk add transaction failed.`);
...
const getSessionQuestionsBySessionId = async (sessionId: number): Promise<SessionQuestion[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionQuestions] Error getting session questions for session ${sessionId}: ${error}`);
      throw error;
    }
  });
};
...
const deleteSessionQuestionsBySessionId = async (sessionId: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionQuestions] Error deleting session questions for session ${sessionId}: ${error}`);
      throw error;
    }
  });
};
...
const addSessionBoitier = async (sb: Omit<SessionBoitier, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionBoitiers] Error adding session boitier: ${error}`);
...
const addBulkSessionBoitiers = async (boitiers: Omit<SessionBoitier, 'id'>[]): Promise<(number | undefined)[] | undefined> => {
  ...
  return asyncDbRun(() => {
    ...
    const transaction = getDb().transaction((items: Omit<SessionBoitier, 'id'>[]) => {
      for (const boitier of items) {
        try {
          ...
          log(`[DB SessionBoitiers] Error in bulk adding session boitier for item: ${boitier}, ${error}`);
          insertedIds.push(undefined);
          throw error; // Rollback transaction
        }
      }
    ...
    try {
      ...
      log(`[DB SessionBoitiers] Bulk add transaction failed.`);
      return undefined;
    }
  });
};
...
const getSessionBoitiersBySessionId = async (sessionId: number): Promise<SessionBoitier[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionBoitiers] Error getting session boitiers for session ${sessionId}: ${error}`);
      throw error;
    }
  });
};
...
const deleteSessionBoitiersBySessionId = async (sessionId: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB SessionBoitiers] Error deleting session boitiers for session ${sessionId}: ${error}`);
      throw error;
    }
  });
};
...
const addReferential = async (referential: Omit<Referential, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Referentiels] Error adding referential: ${error}`);
      if ((error as any).code === 'SQLITE_CONSTRAINT_UNIQUE') {
...
const getAllReferentiels = async (): Promise<Referential[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Referentiels] Error getting all referentiels: ${error}`);
      throw error;
    }
  });
};
...
const getReferentialByCode = async (code: string): Promise<Referential | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Referentiels] Error getting referential by code ${code}: ${error}`);
      throw error;
    }
  });
};
...
const getReferentialById = async (id: number): Promise<Referential | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Referentiels] Error getting referential by id ${id}: ${error}`);
      throw error;
    }
  });
};
...
const addTheme = async (theme: Omit<Theme, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Themes] Error adding theme: ${error}`);
      if ((error as any).code === 'SQLITE_CONSTRAINT_UNIQUE') {
...
const getAllThemes = async (): Promise<Theme[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Themes] Error getting all themes: ${error}`);
      throw error;
    }
  });
};
...
const getThemesByReferentialId = async (referentialId: number): Promise<Theme[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Themes] Error getting themes by referentialId ${referentialId}: ${error}`);
      throw error;
    }
  });
};
...
const getThemeByCodeAndReferentialId = async (code_theme: string, referentiel_id: number): Promise<Theme | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Themes] Error getting theme by code ${code_theme} and referentialId ${referentiel_id}: ${error}`);
      throw error;
    }
  });
};
...
const getThemeById = async (id: number): Promise<Theme | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Themes] Error getting theme by id ${id}: ${error}`);
      throw error;
    }
  });
};
...
const addBloc = async (bloc: Omit<Bloc, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Blocs] Error adding bloc: ${error}`);
      if ((error as any).code === 'SQLITE_CONSTRAINT_UNIQUE') {
...
const getAllBlocs = async (): Promise<Bloc[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Blocs] Error getting all blocs: ${error}`);
      throw error;
    }
  });
};
...
const getBlocsByThemeId = async (themeId: number): Promise<Bloc[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Blocs] Error getting blocs by themeId ${themeId}: ${error}`);
      throw error;
    }
  });
};
...
const getBlocByCodeAndThemeId = async (code_bloc: string, theme_id: number): Promise<Bloc | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Blocs] Error getting bloc by code ${code_bloc} and themeId ${theme_id}: ${error}`);
      throw error;
    }
  });
};
...
const getBlocById = async (id: number): Promise<Bloc | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB Blocs] Error getting bloc by id ${id}: ${error}`);
      throw error;
    }
  });
};
...
const addDeviceKit = async (kit: Omit<DeviceKit, 'id'>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      log(`[DB DeviceKits] Attempting to add device kit: ${kit}`);
      ...
      log(`[DB DeviceKits] Successfully added device kit with ID: ${result.lastInsertRowid}`);
      return result.lastInsertRowid as number;
    } catch (error) {
      log(`[DB DeviceKits] Error adding device kit: ${error}`);
      if ((error as any).code === 'SQLITE_CONSTRAINT_UNIQUE') {
...
const getAllDeviceKits = async (): Promise<DeviceKit[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKits] Error getting all device kits: ${error}`);
      throw error;
    }
  });
};
...
const getDeviceKitById = async (id: number): Promise<DeviceKit | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKits] Error getting device kit by id ${id}: ${error}`);
      throw error;
    }
  });
};
...
const updateDeviceKit = async (id: number, updates: Partial<Omit<DeviceKit, 'id'>>): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKits] Error updating device kit ${id}: ${error}`);
...
const deleteDeviceKit = async (id: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKits] Error deleting device kit ${id}: ${error}`);
      throw error;
    }
  });
};
...
const getDefaultDeviceKit = async (): Promise<DeviceKit | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKits] Error getting default device kit: ${error}`);
      throw error;
    }
  });
};
  ...
  return asyncDbRun(() => {
    const transaction = getDb().transaction(() => {
      try {
        ...
        if (result.changes === 0) {
          log(`[DB DeviceKits] setDefaultDeviceKit: Kit with id ${kitId} not found or no change made.`);
        ...
        log(`[DB DeviceKits] Error setting default device kit ${kitId}: ${error}`);
        throw error; // Annule la transaction
      }
    });
    transaction();
...
const assignDeviceToKit = async (kitId: number, votingDeviceId: number): Promise<number | undefined> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKitAssignments] Error assigning device ${votingDeviceId} to kit ${kitId}: ${error}`);
      if ((error as any).code === 'SQLITE_CONSTRAINT_UNIQUE') {
...
const removeDeviceFromKit = async (kitId: number, votingDeviceId: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKitAssignments] Error removing device ${votingDeviceId} from kit ${kitId}: ${error}`);
      throw error;
    }
  });
};
...
const getVotingDevicesForKit = async (kitId: number): Promise<VotingDevice[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKitAssignments] Error getting voting devices for kit ${kitId}: ${error}`);
      throw error;
    }
  });
};
...
const getKitsForVotingDevice = async (votingDeviceId: number): Promise<DeviceKit[]> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKitAssignments] Error getting kits for voting device ${votingDeviceId}: ${error}`);
      throw error;
    }
  });
};
...
const removeAssignmentsByKitId = async (kitId: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKitAssignments] Error removing assignments by kitId ${kitId}: ${error}`);
      throw error;
    }
  });
};
...
const removeAssignmentsByVotingDeviceId = async (votingDeviceId: number): Promise<void> => {
  return asyncDbRun(() => {
    try {
      ...
      log(`[DB DeviceKitAssignments] Error removing assignments by votingDeviceId ${votingDeviceId}: ${error}`);
      throw error;
    }
  });
};
    ...
    for (const session of sessionsForBlockUsage) {
      ...
      try {
        ...
        if (Array.isArray(parsedBlocIds) && parsedBlocIds.every(id => typeof id === 'number')) {
          ...
          log(`[DB Reports] Session ${session.id} has malformed selectedBlocIds: ${session.selectedBlocIds}`);
          continue;
        }
      } catch (e) {
        log(`[DB Reports] Session ${session.id} failed to parse selectedBlocIds: ${session.selectedBlocIds}, ${e}`);
        continue;
      }
      ...
      for (const blocId of blocIds) {
        try {
          ...
          if (blocDetails) {
            ...
            log(`[DB Reports] Block details not found for blocId ${blocId} in referentielId ${session.referentielId} for session ${session.id}`);
          }
        } catch (e) {
            log(`[DB Reports] Error processing blocId ${blocId} for session ${session.id}: ${e}`);
        }
      }
    }
